...
public class Panel {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		int outlineColor = new Color(255,255,255,255).getRGB();
		
		Gui.drawRect(x, y, x + width, y + height, 0xff121212);
		if(Client.instance.settingsManager.getSettingByName("Clickgui.Design").getValString().equalsIgnoreCase("New")){
			Gui.drawRect(x, y, x + 2, y + height, outlineColor);
			FontUtil.drawTotalCenteredStringWithShadow(title, x + width / 2, y + height / 2, 0xffefefef);
			Gui.drawRect(x - 2 + width , y, x  + width, y + height, outlineColor);
		}else if(Client.instance.settingsManager.getSettingByName("Clickgui.Design").getValString().equalsIgnoreCase("JellyLike")){
		...
		if (this.extended && !Elements.isEmpty()) {
			...
			int epanelcolor = Client.instance.settingsManager.getSettingByName("Clickgui.Design").getValString().equalsIgnoreCase("New") ? 0xff232323 : Client.instance.settingsManager.getSettingByName("Clickgui.Design").getValString().equalsIgnoreCase("JellyLike") ? 0xbb151515 : 0;;
			for (ModuleButton et : Elements) {
				if(Client.instance.settingsManager.getSettingByName("Clickgui.Design").getValString().equalsIgnoreCase("New")){
					//	Gui.drawRect(x - 2, startY, x + width, startY + et.height + 1, outlineColor);
					Gui.drawRect(x - 2 + width , startY, x  + width, startY + et.height + 1, outlineColor);
				}
